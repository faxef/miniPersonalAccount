...
export class TicketPageComponent implements OnDestroy {
  ...
  private ticketParamsSubscription: any;
  ...
  ngOnInit() {
    ...
    this.ticketParamsSubscription = this.activatedRoute.params.subscribe(params => {
  ...
  ticketDelete(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        // Если удаление подтверждено, вызываем метод удаления из сервиса
  ...
  getTicketData() {
    ...
    ).subscribe().unsubscribe(); // Отписка от потока после получения данных
  }
  ...
  ngOnDestroy(): void {
    this.ticketParamsSubscription.unsubscribe(); // Отписка от параметров маршрута при уничтожении компонента
  }
}
